...
def preprocess(train, val, test, normalization='quantile', target="target_cox", norm_target: bool = False, seed=42):
    ...
    cat_features = list(train
                    ...
                    .drop(columns=passthrough_cols, errors='ignore')
                    .columns)
    num_features = list(train
    ...
    def add_noise(df, cols, size):
        df2 = df.copy()
        df2[cols] = df2[cols] + np.random.default_rng(1).normal(0.0, 1e-5, (size, len(cols)))
        return df2

    # don't forget to add noise for fitting
    if normalization == 'quantile':
        ft.fit(train.pipe(add_noise, num_features, len(train)))
    else:
        ft.fit(train)
    
    if norm_target:
        regression_label_stats = RegressionLabelStats(
            train[target].mean().item(), train[target].std().item()
        )
    ...
    for part_name, part in zip(["train", "val", "test"], [train, val, test]):
        if part is not None:
            ...
            # need to fix column names after Feature Union transformer
            prep = prep.rename(columns={col: col[3:] for col in prep.columns if col.startswith("X") or col.startswith("Y")} )
            prep.to_csv(f"{part_name}.csv", index=False)
            data[part_name] = {'x_cont': prep[num_features].to_numpy().astype(np.float32), 
                               'x_cat': prep[cat_features].to_numpy().astype(np.int64),
                               'y': target_transformer.transform(prep[target].to_numpy()).flatten(),
                               'metric': prep[['ID', 'efs', 'efs_time', 'race_group']]
                               }
